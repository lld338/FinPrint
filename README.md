...
FinPrint 是一个在浏览器中运行的 A4/A5 报销材料拼版、预览、导出与打印工具。编辑时，A4/A5 决定逻辑版面尺寸，横向/纵向只改变纸张方向，原稿文字和 PDF 内容不会自动旋转；导出时所有逻辑版面统一放到 A4 承载页。A5 纵版放到 A4 纵向页左上角，A5 横版放到 A4 横向页左上角（均为左齐、顶齐），便于报销打印后裁切与装订。它支持 PDF、JPG、JPEG、PNG、WEBP 和 BMP，并可按整页、上下两联或左右两联方式拼版。
...
- **智能排版只根据页面尺寸**，不按文件名做任何业务预设。
- 支持 A4/A5 纵向和横向四种独立逻辑版面，切换纸张或方向时不旋转原稿。
- 实际输出页统一为 A4：A5 竖版放在 A4 纵向页左上角，A5 横版放在 A4 横向页左上角（左齐、顶齐）。
- 支持整页、上下两联、左右两联三种版式。
- 支持调整分割比例、页边距和中间留白。
...
- 支持靠左、居中、靠右快捷定位（控制内容在版位内的位置，不是 A5 在 A4 上的位置）。
...
只读取 PDF **页面框**宽高（pt），与标准 A4 / A5 比较（约 8% 容差），**不读取文件名**：

| 页面框 | 默认结果 |
|--------|----------|
| 接近 A5（约 148×210 mm） | A5 **整页**，方向跟随原稿（宽 > 高 → 横向） |
| 接近 A4（约 210×297 mm） | A4 **整页**，方向跟随原稿 |
| 其它尺寸（含图片转换页） | 两两放入 **A4 上下两联** |

卡片上的「PDF 页面 A4 / A5」只表示原稿页面框，不等于你最终选的逻辑纸张；可在「03 · 打印设置」里改。

### 编辑 vs 导出

| 阶段 | 行为 |
|------|------|
| 编辑 / 预览 | 使用真正的 A4 或 A5 逻辑尺寸；方向只交换宽高，**不旋转**原稿内容 |
| 导出 PDF | 每一物理页都是 **A4**；逻辑 A4 铺满；逻辑 A5 按真实尺寸贴在对应方向的 A4 **左上角**（左齐、顶齐） |
| 版位内适配 | 默认「完整显示」等比塞进版位；「铺满区域」可拉伸。「水平位置」只挪版位内的内容 |
| 裁切 | 使用 PDF.js 可见页面框（view）；可选自动去掉下方大面积空白 |

打印时请选 **A4 + 实际大小 / 100%**，不要「适合页面」。
...
PDF 文件卡片中的“PDF 页面 A4 / A5”表示原稿 PDF 页面框的实际尺寸，不代表最终选择的逻辑版面，也不代表页面中的有效内容占用了整张纸。图片没有可靠的物理尺寸，文件卡片会显示“图片 JPG / PNG”等格式；导入后可在右侧选择 A4/A5、纵向/横向，并设置整页、上下或左右排版。

例如，一个 PDF 的内容可能只在上半页，但它的页面框仍然是 A4，此时仍会显示“PDF 页面 A4”。是否裁掉下方空白，应在右侧“原稿空白处理”中设置。

### 02 · 拼版预览

用于查看当前选择的逻辑纸张和版位内容。预览会分别显示 A4 纵向、A4 横向、A5 纵向或 A5 横向的真实比例；A5 的 A4 承载页位置只在最终导出阶段应用。可以：
...
4. A4 和 A5 逻辑页面可以放在同一个 PDF 中连续打印，因为生成的每一页实际都是 A4；A5 竖版位于 A4 纵向页左上角，A5 横版位于 A4 横向页左上角（左齐、顶齐）；
5. A4 / A5 打印尺寸的整页版式默认使用 `0 mm` 页边距，避免内容被再次缩小。需要为打印机保留白边时，可在“03 · 打印设置”中手动增加页边距。
...
如果把 A4 竖版原稿放到 A5 横版逻辑版面，系统只会按当前版位等比例适配，不会旋转原稿文字。导出时，完整的 A5 横版（210 × 148 mm）会放到 A4 横向页面左上角（左齐、顶齐）。A5 竖版则以 148 × 210 mm 放到 A4 纵向页面左上角。打印时选择 A4 和“实际大小 / 100%”，A4 与 A5 版面可以在同一个文件中一次打印，不需要分纸或切换纸盒。

### 6. 图片无法导入或方向不正确

当前支持 JPG、JPEG、PNG、WEBP 和 BMP。HEIC、TIFF、SVG 等格式暂不支持，请先使用系统图片工具转换为 JPG 或 PNG。

图片导入后会显示为单页材料，并默认参与 A4 上下拼版。图片本身通常没有可信的 A4 / A5 物理尺寸，可在“03 · 打印设置”中手动选择 A4/A5、纵向/横向、整页 / 上下 / 左右，并调整缩放和位置。
