...
FinPrint 是一个在浏览器中运行的 A4 报销材料拼版、预览、导出与打印工具。实际打印纸张统一为 A4，“A4 / A5”表示内容的打印方式：既可以按 A4 整页输出，也可以把竖版原稿自动旋转后，按 A5 横版尺寸放在 A4 纵向纸的上半页。它支持 PDF、JPG、JPEG、PNG、WEBP 和 BMP，并可按整页、上下两联或左右两联方式拼版。
...
- 实际输出页统一为 A4；A5 模式使用 A4 纵向纸，并把 A5 横版内容放在上半页。
- 支持 A4 纸纵向和横向输出。
- 支持整页、上下两联、左右两联三种版式。
- 支持调整分割比例、页边距和中间留白。
...
PDF 文件卡片中的“PDF 页面 A4 / A5”表示原稿 PDF 页面框的实际尺寸，不代表最终使用哪种纸，也不代表页面中的有效内容占用了整张纸。图片没有可靠的物理尺寸，文件卡片会显示“图片 JPG / PNG”等格式；导入后可在右侧选择“A4 整页”或“A5 横版居左（上半页）”，并设置整页、上下或左右排版。

例如，一个 PDF 的内容可能只在上半页，但它的页面框仍然是 A4，此时仍会显示“PDF 页面 A4”。是否裁掉下方空白，应在右侧“原稿空白处理”中设置。

### 02 · 拼版预览

用于查看当前 A4 打印纸和版位内容。选择“A5 横版居左（上半页）”时，预览会显示 210 × 148 mm 的 A5 横版区域位于 A4 纵向纸上半页的实际位置。可以：
...
4. A4 整页和 A5 横版页面可以放在同一个 PDF 中连续打印，因为生成的每一页实际都是 A4；A5 页面会输出为 A4 纵向，上半页为 A5 横版区域；
...
如果原稿是 A4 竖版，但在 FinPrint 中选择“A5 横版居左（上半页）”，系统会把原稿自动旋转 90°，按 A4 到 A5 的比例缩小到 A5 横版区域（210 × 148 mm），并放在 A4 纵向输出页的上半页，下半页保留空白。打印时选择 A4、纵向和“实际大小 / 100%”。A4 整页与 A5 横版可以在同一个文件中一次打印，不需要分纸或切换纸盒。

### 6. 图片无法导入或方向不正确

当前支持 JPG、JPEG、PNG、WEBP 和 BMP。HEIC、TIFF、SVG 等格式暂不支持，请先使用系统图片工具转换为 JPG 或 PNG。

图片导入后会显示为单页材料，并默认参与 A4 上下拼版。图片本身通常没有可信的 A4 / A5 物理尺寸，可在“03 · 打印设置”中手动选择 A4 整页或 A5 横版居左（上半页）、方向、整页 / 上下 / 左右，并调整缩放和位置。
